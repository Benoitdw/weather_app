...
const Hours = ({ dateToFetch }: HourProps) => {
  ...
  useEffect(() => {
    ...
    meteo?.hourly.time.forEach((value, index) =>{
      ...
      if (valueDate.getDate() == dateToFetch.getDate()){
          let details = {"date": valueDate, "weather_icon": "./assets/weather_icons/" + VMO[meteo.hourly.weathercode[index]].icon, "temperature": meteo.hourly.temperature_2m[index] };
  ...
  return (
    <div className="overflow-x-auto mx-4 mt-4 flex gap-2">
      {hours.length != 0 ? hours.map((value, index) => {
          return (
        <div className="bg-white/30 snap-center shrink-0 w-[40px] h-[70px] rounded-full flex flex-col content-center items-center text-[#9C9EAA] text-xs justify-center py-1">
          ...
          <div className="w-6 overflow-hidden -mt-1 ">
            <img src={value.weather_icon} className="transform scale-[2.5]" />
          </div>
          <div className="text-primary font-bold">{value.temperature}°</div>
        </div>
        )
        }):
        "Loading"}
...
const NextDays = () => {
  ...
  return (
    <>
      <div className="flex text-3 gap-3 px-4 text-secondary text-sm">
        <Day
      ...
      <div className="mx-4 mt-2 h-[1px] bg-secondary" />
